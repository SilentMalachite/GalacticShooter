...
function updateParticles(deltaTime) {
  for (let i = gameConfig.particles.length - 1; i >= 0; i--) {
    const p = gameConfig.particles[i];
    p.x += p.vx;
    p.y += p.vy;
    p.life--;
    if (p.life <= 0) {
      gameConfig.particles.splice(i, 1);
    }
  }
}

export { spawnWave, updatePlayer, updateEnemies, updateBullets, shootPlayerBullet, checkCollisions, createBombExplosion, isColliding, createParticle, updateParticles };
